...
function parseFrame(text: string): StackFrame | null {
    ...
    if (s) {
        return { raw: text, dotPath: s[1], line: Number(s[2]), label: s[3] ?? "" };
    }
    const i = FRAME_INLINE.exec(text);
    if (i) {
        return { raw: text, dotPath: i[1], line: Number(i[2]), label: "" };
    }
    return null;
}
...
export function groupMessages(messages: RuntimeMessage[]): GroupedEntry[] {
    ...
    while (i < messages.length) {
        ...
        if (msg.type === "error") {
            // Collect the group: header + frames
